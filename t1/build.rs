...
/// This crate has a runtime dependency on artifacts produced by the `t1-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "t1-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `t1-ebpf`, so cargo will automatically rebuild us
        // if `t1-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `t1-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("t1-ebpf");
